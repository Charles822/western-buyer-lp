...
import type { VoiceContactModal } from '@/lib/voice-agent-landing-content';

type ContactChatModalProps = {
  open: boolean;
  onClose: () => void;
  contactModal: VoiceContactModal;
};

export function ContactChatModal({ open, onClose, contactModal }: ContactChatModalProps) {
  const titleId = useId();
      ...
      >
        ...
        {contactModal.kind === 'wechat_qr' ? (
          ...
          <>
            ...
            <p className="mt-2 text-sm leading-relaxed text-zinc-400">
              Open WhatsApp to message Sam—we&apos;ll reply there.
            </p>
            <a
              href={contactModal.href}
              target="_blank"
              rel="noopener noreferrer"
              className="mt-6 flex w-full items-center justify-center gap-2.5 rounded-xl bg-[#25D366] px-4 py-3.5 text-base font-semibold text-white shadow-lg shadow-emerald-950/40 transition hover:bg-[#20bd5a]"
            >
              <Icon icon="logos:whatsapp-icon" className="size-7 shrink-0" aria-hidden />
              Chat on WhatsApp
            </a>
          </>
        )}
